...
export default async function SincronizaSituacao(){
    ...
    try {
        ...
        data_off.forEach(async (item) =>{
            ...
            if(!result){
                const result = servicesSituation('DELETE', 'situacao', '', item.codsit, '', '');
                console.log(result)
            }
        ...
        if (data.length > 0) {
            data.forEach((item) => {
            const result = servicesSituation('SELECT', 'situacao', 'ID', item.codsit, '', '');
            result.then((value)=>{
                if(value.length > 0){
                    const result = servicesSituation('UPDATE', 'situacao', '', item, '', '');
                    console.log(result)
                }
                else {
                    const result = servicesSituation('INSERT', 'situacao', '', item, '', '');
                }
            })
            });
        }
